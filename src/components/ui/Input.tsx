...
export const Input = forwardRef<HTMLInputElement, InputHTMLAttributes<HTMLInputElement> & { mono?: boolean }>(
  function Input({ className, mono, ...rest }, ref) {
    return <input ref={ref} className={cn(base, 'h-10', mono && 'font-mono text-[13px]', className)} {...rest} />
  },
)
...
>(function TextArea({ className, mono, ...rest }, ref) {
  return (
    <textarea
      ref={ref}
      spellCheck={false}
      className={cn(base, 'thin-scrollbar min-h-32 resize-y py-3 leading-relaxed', mono && 'font-mono text-[13px]', className)}
      {...rest}
    />
  )
})
